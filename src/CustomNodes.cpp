...
{
    //return VX_SUCCESS;
    if(num != 9) return VX_ERROR_INVALID_PARAMETERS;
...

    {
        ...
        for (i = 0; i < sz; i++)
        {
            ...
            if(tmpKp[currentIdx].strength < newfast_value) continue; //not corner in first metric
        ...
        {
            tmpKp[currentIdx] = vxArrayItem(vx_keypoint_t, base, i, stride);
            if(tmpKp[currentIdx].strength >= newfast_value) continue; //corner already processed in first metric
...
vx_status scaleArray_kernel(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
    ...
    vx_array kp_in  = (vx_array) parameters[0];
    vx_float32 mult; NVXIO_SAFE_CALL(vxCopyScalar((vx_scalar) parameters[1], &mult, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
...
vx_convolution createGaussianConvolution(vx_context c, vx_size size, vx_float32 sigma, vx_uint32 scale, bool horizontal)
{
    ...
    int y = 0;
    {
        if(horizontal) for(; idx < size; ++idx) tmp[idx] = 0;
        ...
        if(horizontal) for(; idx < 3*size; ++idx) tmp[idx] = 0;
        //std::cout << std::endl;
    }
