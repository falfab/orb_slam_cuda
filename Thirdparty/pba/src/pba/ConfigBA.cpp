...
void ConfigBA::PrintBundleStatistics()
{
    ...
    outfile.open("../../times/pba.csv", std::ios_base::app);
    outfile << ((int)(BundleTimerGet(TIMER_OVERALL) * 1000)) << ";" << ((int)(BundleTimerGet(TIMER_OPTIMIZATION) * 1000)) << ";" << std::endl;
    outfile.close();
}
...
#define MAKEINT4(a, b, c, d) (MAKEINT1(a) + (MAKEINT3(b, c, d) << 8))
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        case MAKEINT4(n, o, r, m):
            ...
            break;
        case MAKEINT3(d, c, e):
            if (i + 1 < argc && sscanf(param, "%f", &argf) && argf > 0 && argf <= 0.01)
