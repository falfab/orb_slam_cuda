...
{
    if (__selected_device != __current_device)
        SetCudaDevice(__selected_device);
    size_t sz = ProgramCU::GetCudaMemoryCap();
    if (sz < 1024)
        std::cout << "ERROR: CUDA is unlikely to be supported!\n";
    return sz < 1024 ? 0 : sz;
}

void SparseBundleCU::SetCameraData(size_t ncam, CameraT *cams)
{
    if (sizeof(CameraT) != 16 * sizeof(float))
        exit(0); //never gonna happen...?
...
{
    if (ValidateInputData() != STATUS_SUCCESS)
        return;
...
bool SparseBundleCU::InitializeBundleGPU()
{
    ...
    {
        if (__verbose_level)
            std::cout << "WARNING: try clean allocation\n";
        ClearPreviousError();
    ...
    {
        if (__verbose_level)
            std::cout << "WARNING: try not storing original JC\n";
    ...
    {
        if (__verbose_level)
            std::cout << "WARNING: try not storing transpose JC\n";
    ...
    {
        if (__verbose_level)
            std::cout << "WARNING: switch to memory saving mode\n";
...
{
    if (CheckRequiredMem(0))
        return true;
    if (__jc_store_original)
    {
        if (__verbose_level)
            std::cout << "NOTE: not storing original JC\n";
        __jc_store_original = false;
        if (CheckRequiredMem(1))
            return true;
    }
    if (__jc_store_transpose)
    {
        if (__verbose_level)
            std::cout << "NOTE:  not storing camera Jacobian\n";
        __jc_store_transpose = false;
        if (CheckRequiredMem(1))
            return true;
    }
    ...
    {
        if (__verbose_level)
            std::cout << "NOTE: not storing any Jacobian\n";
        __no_jacobian_store = true;
        if (CheckRequiredMem(1))
            return true;
    }
...
void SparseBundleCU::ReserveStorage(size_t ncam, size_t npt, size_t nproj)
{
    ...
    else
    {
        ...
        if (__verbose_level)
            std::cout << "Reserving storage for ncam = " << ncam << "; npt = "
                      << npt << "; nproj = " << nproj << '\n';
        InitializeBundleGPU();
...
{
    if (_cuCameraData.data() == NULL || _cuPointData.data() == NULL || _cuMeasurements.data() == NULL)
        return;
...
bool SparseBundleCU::TransferDataToGPU()
{
    ...
    if (_camera_idx && _point_idx)
    {
        ...
        for (int i = 0; i < _num_imgpt; ++i)
            cpnum[_camera_idx[i]]++;
        for (int i = 1; i <= _num_camera; ++i)
            cpi[i] = cpi[i - 1] + cpnum[i - 1];
        vector<int> cptidx = cpi;
        for (int i = 0; i < _num_imgpt; ++i)
            cpidx[cptidx[_camera_idx[i]]++] = i;
        if (_num_imgpt_q > 0)
            ProcessWeightCameraQ(cpnum, qmap, qmapw, qlistw);
        BundleTimerSwap(TIMER_PREPROCESSING, TIMER_GPU_ALLOCATION);
    ...
    if (__verbose_level > 1)
        std::cout << "Memory for Motion/Structure/Jacobian:\t" << (total_sz / 1024 / 1024) << "MB\n";
    return success;
}
...
void SparseBundleCU::NormalizeDataD()
{
    ...
    if (__depth_scaling == 1.0f)
    {
                ...
                else if ((cpdist1[i] < dist_threshold || cpdist2[i] > -dist_threshold))
                {
                    ...
                    if (__verbose_level > 3)
                        std::cout << "Adjust C" << std::setw(5) << i << " by " << std::setw(12) << shift
                                  << " [B" << std::setw(2) << cambpj[i] << "/" << std::setw(5) << camnpj[i] << "] [" << (boths ? 'X' : ' ') << "][" << cpdist1[i] << ", " << cpdist2[i] << "]\n";
        ...
        if (__num_point_behind > 0)
            std::cout << "WARNING: " << __num_point_behind << " points are behind camras.\n";
        if (__num_camera_modified > 0)
            std::cout << "WARNING: " << __num_camera_modified << " camera moved to avoid degeneracy.\n";
    }
}
...
bool SparseBundleCU::InitializeStorageForCG()
{
    ...
    __memory_usage += total_sz;
    if (__verbose_level > 1)
        std::cout << "Memory for Conjugate Gradient Solver:\t" << (total_sz / 1024 / 1024) << "MB\n";
    return success;
}
...
void SparseBundleCU::EvaluateJacobians(bool shuffle)
{
    if (__no_jacobian_store)
        return;
    if (__bundle_current_mode == BUNDLE_ONLY_MOTION && !__jc_store_original && !__jc_store_transpose)
        return;
...
void SparseBundleCU::ComputeJtE(CuTexImage &E, CuTexImage &JtE, int mode)
{
    ...
    if (__no_jacobian_store || (!__jc_store_original && !__jc_store_transpose))
    {
        ...
        else if (mode == 2)
        {
            ...
            ComputeVXY(JtE, _cuVectorSJ, JtE, _cuJacobianPoint.IsValid() ? _num_camera * 8 : 0);
    }
    ...
    if (mode != 2 && _num_imgpt_q > 0)
        ProgramCU::ComputeJQtEC(E, _cuCameraQList, _cuCameraQListW, _cuVectorSJ, JtE);
}
    ...
    if (__no_jacobian_store || (__multiply_jx_usenoj && mode != 2) || !__jc_store_original)
    {
        if (_cuVectorSJ.IsValid())
        {
            if (mode == 0)
                ...
                ProgramCU::ComputeVXY(X, _cuVectorSJ, _cuVectorZK, _num_point * 4, _num_camera * 8);
...
void SparseBundleCU::ComputeDiagonal(CuTexImage &JJ, CuTexImage &JJI)
{
    ...
    if (__jc_store_transpose)
    {
        ProgramCU::ComputeDiagonal(_cuJacobianCameraT, _cuCameraMeasurementMap, _cuJacobianPoint, _cuPointMeasurementMap, _cuCameraMeasurementList,
                                   JJ, JJI, true, __use_radial_distortion, use_diagonal_q);
    }
    else
    {
        ProgramCU::ComputeDiagonal(_cuJacobianCamera, _cuCameraMeasurementMap, _cuJacobianPoint, _cuPointMeasurementMap, _cuCameraMeasurementList,
...
int SparseBundleCU::SolveNormalEquationPCGX(float lambda)
{
    ...
    if (__verbose_cg_iteration)
        std::cout << " --0,\t alpha = " << alpha0 << ", t = " << BundleTimerGetNow(TIMER_CG_ITERATION) << "\n";
    if (!finite(alpha0))
    {
        return 0;
    }
    ...
    while (true)
    {
        ...
        if (rtz_ratio < __cg_norm_threshold)
        {
            if (__recent_cg_status == ' ')
                __recent_cg_status = iteration < std::min(10, __cg_min_iteration) ? '0' + iteration : 'N';
            if (iteration >= __cg_min_iteration)
                break;
        }
        ////////////////////////////////////////////////////////////////////////////
        ...
        ///////////////////////////////////////////////////
        if (!finite(alphak) || rtz_ratio > __cg_norm_guard)
        {
        ...
        ++iteration;
        ++__num_cg_iteration;
        if (iteration >= std::min(__cg_max_iteration, plen))
            break;
...
void SparseBundleCU::NonlinearOptimizeLM()
{
    ...
    for (int i = 0; i < __lm_max_iteration && !__abort_flag; __current_iteration = (++i))
    {
        ...
        if (finite(new_residual) && residual_reduction > 0)
        {
            ...
            {
                if (__verbose_level > 1)
                    std::cout << "#" << std::setw(3) << i << " converged with small gradient\n";
                __pba_return_code = 'G';
                break;
            }
            ...
            else
            {
                ...
                else if (__lm_damping_auto_switch == 0 && damping > __lm_maximum_damp && __lm_use_diagonal_damp)
